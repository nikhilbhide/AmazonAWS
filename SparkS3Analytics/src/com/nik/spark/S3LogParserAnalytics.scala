...
object S3LogParserAnalytics {
  ...
  def getTopNIPAddressForSelectedBuckets(inputBucketName: String, n: Int) {
    ...
    val topNIPForSelectedBuckets = initSpark().sqlContext.sql("select ipAddress,count(ipAddress) as Hits from s3Logs group by ipAddress having ipAddress in (select ipAddress from s3Logs where bucketName in ($inputBucketName))")
